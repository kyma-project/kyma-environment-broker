...
Currently, there is one task for Runtime Reconciler. It reconciles BTP Manager Secrets on SKRs. It can do it in two ways: 
- by implementation with the usage of Runtime Watcher, which sends events about changes of the Secret from an SKR to KEB in real time. 
