...
Versioned machine types such as `m6i.large` are still supported. However, it is recommended to mark them as deprecated in the schema and switch to the corresponding version-agnostic names.
...
For full provider-specific configuration examples and machine version resolution tables, see [Version-Agnostic Machine Types](version-agnostic-machine-types.md).
