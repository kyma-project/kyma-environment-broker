...
To reduce this coupling, machine names are partially abstracted. Instead of requiring full provider-specific instance names, the configuration uses version-agnostic names such as `mi.large` (AWS) that KEB resolves to the current provider-specific instance at provisioning and update time.
...
> ### Note:
> Kyma Environment Broker (KEB) does not automatically reconcile or update existing worker node pools when the machines versions configuration changes.
