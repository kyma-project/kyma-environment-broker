...
| hap.multiHyperscalerAccount.<br>inconsistencyThreshold | Minimum number of claimed CredentialsBindings for a global account that activates the data-inconsistency guard. When this many bindings are claimed but none have active instances in the DB, provisioning returns an error. Set to 0 to disable the guard. | `0` |
