...
These parameters are fields of the `ProvisioningParametersDTO` struct in [`common/runtime/model.go`](../github.com/kyma-project/kyma-environment-broker/common/runtime/model.go). They are used internally by Kyma Environment Broker (KEB) but are not exposed to users using the Open Service Broker (OSB) provisioning API.
