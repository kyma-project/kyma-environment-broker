...
Users can also request extra disk space on top of the default by setting the optional `additionalVolumeSizeGi` parameter. KEB computes the total volume size as `defaultVolumeSize + additionalVolumeSizeGi`.
...
- Kyma worker pool: If the machine type changes or `additionalVolumeSizeGi` changes, the volume is recomputed as `base + additionalVolumeSizeGi`. If neither changes, the existing volume is preserved.
