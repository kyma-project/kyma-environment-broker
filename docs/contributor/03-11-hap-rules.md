...
> This feature is still being developed and will be available soon.
 ...
 The process generates a list of [search labels](#search-labels) based on skr's physical attributes.
...
Rule entry attributes and its plan alter the search query by adding new or modifing existing [search labels](#search-labels). 
Existence of an attribute in the rule means its inclusion in the [search labels](#search-labels) in addition to labels associated with rule plan.
...
* Name and symbol - the later is used in rule entries.
