...
func (reconciler *stateReconcilerType) recreateStateFromDB() {
	logs := reconciler.logs
	dbStates, err := reconciler.db.SubaccountStates().ListStates()
	if err != nil {
		logs.Error(fmt.Sprintf("error while fetching states from db: %s", err))
		return
	}
	...
	if err != nil {
		logs.Error(fmt.Sprintf("error while fetching subaccounts from db: %s", err))
		return
	}
	...
	for subaccount := range reconciler.inMemoryState {
		...
		if !ok {
			logs.Warn(fmt.Sprintf("subaccount %s found in previous state but not found in current instances", subaccount))
		}
	}
...
func (reconciler *stateReconcilerType) periodicAccountsSync() {
	...
	logs.Info(fmt.Sprintf("Found %d subaccounts in in-memory state", len(subaccountsSet)))
	...
	for subaccountID := range subaccountsSet {
		...
		if err != nil {
			logs.Error(fmt.Sprintf("error while getting data for subaccount:%s", err))
		} else {
...
func (reconciler *stateReconcilerType) periodicEventsSync(fromActionTime int64) {
	...
	if err != nil {
		logs.Error(fmt.Sprintf("error while getting subaccount events: %s", err))
...
func (reconciler *stateReconcilerType) runCronJobs(cfg Config, ctx context.Context) {
	...
	_, err := s.Every(cfg.EventsWindowInterval).Do(func() {
		...
		logs.Info(fmt.Sprintf("Running CIS events synchronization from epoch: %d", eventsFrom))
	...
	if err != nil {
		logs.Error(fmt.Sprintf("error while scheduling events sync job: %s", err))
	}

	_, err = s.Every(cfg.AccountsSyncInterval).Do(func() {
		logs.Info(fmt.Sprintf("Running full CIS accounts synchronization"))
		reconciler.periodicAccountsSync()
		reconciler.metrics.cisRequests.With(prometheus.Labels{"endpoint": "accounts"}).Inc()
	})
	if err != nil {
		logs.Error(fmt.Sprintf("error while scheduling accounts sync job: %s", err))
	}
	...
	if err != nil {
		logs.Error(fmt.Sprintf("error while scheduling storage sync job: %s", err))
	}
...
func (reconciler *stateReconcilerType) enqueueSubaccountIfOutdated(subaccountID subaccountIDType, state subaccountStateType) {
	if isResourceOutdated(state) {
		reconciler.logs.Info(fmt.Sprintf("Subaccount %s is outdated, enqueuing for sync", subaccountID))
		...
		reconciler.logs.Info(fmt.Sprintf("Subaccount %s is up to date", subaccountID))
	}
}

// TODO extract and create unit tests
// TODO make it shorter
func isResourceOutdated(state subaccountStateType) bool {
	var outdated bool
...
func (reconciler *stateReconcilerType) storeStateInDb() {
	...
	for subaccount, state := range reconciler.inMemoryState {
		if state.pendingDelete { // no runtimes left, we can delete the state from the storage
			...
			if err != nil {
				logs.Error(fmt.Sprintf("error while deleting subaccount:%s state from persistent storage: %s", subaccount, err))
				failureCnt++
				continue
			}
			deleteCnt++
