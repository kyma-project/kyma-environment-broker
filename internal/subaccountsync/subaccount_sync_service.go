...
const (
	...
	metricsPort           = ":8081" //TODO what port? configurable?
)
...
func (s *SyncService) Run() {
	...
	go func() {
		...
		if err != nil {
			logger.Error(fmt.Sprintf("error while serving metrics: %s", err))
		}
	}()
	...
	if s.cfg.UpdateResources {
		updater, err = kymacustomresource.NewUpdater(s.k8sClient, priorityQueue, s.kymaGVR)
		fatalOnError(err)
	}
	...
	stateReconciler := stateReconcilerType{
		...
		logs:             logger.With("service", "state-reconciler"),
		db:               s.db,
		updater:          updater,
	...
	configureInformer(&informer, logrus.New(), &stateReconciler, logger, metrics)
	...
	if s.cfg.UpdateResources && stateReconciler.updater != nil {
		go func() {
			...
			if err != nil {
				logger.Warn(fmt.Sprintf("error while running updater: %s, cannot update", err))
			}
		}()
	} else {
...
func CreateEventsClient(ctx context.Context, eventsConfig CisEndpointConfig, logs *slog.Logger) *RateLimitedCisClient {
	eventsClient := NewRateLimitedCisClient(ctx, eventsConfig, logs.With("client", "CIS-Events"))
	return eventsClient
}

func getDataFromLabels(obj interface{}) (subaccountID string, runtimeID string, betaEnabled string) {
