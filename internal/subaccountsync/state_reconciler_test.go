...
func TestStateReconciler(t *testing.T) {
	...
	t.Run("should schedule update of one resource after getting account data from CIS", func(t *testing.T) {
		...
		//TODO usedForProduction with proper literal values
		// then we get state from CIS (accounts)
		reconciler.reconcileCisAccount(subaccountId1, CisStateType{BetaEnabled: true, UsedForProduction: "false", ModifiedDate: veryOldTime})
	...
	t.Run("should schedule update of one resource after getting account and event from CIS", func(t *testing.T) {
		...
		reconciler.reconcileCisAccount(subaccountId1, CisStateType{BetaEnabled: false, UsedForProduction: "false", ModifiedDate: 1})
	...
	t.Run("should update resource after restart regardless of lost event", func(t *testing.T) {
		...
		reconciler.reconcileCisAccount(subaccountId1, CisStateType{BetaEnabled: false, UsedForProduction: "NOT_USED", ModifiedDate: veryOldTime})
	...
	t.Run("should update resource before restart", func(t *testing.T) {
		...
		// initial add event from the kyma resource, first runtime, no label
		reconciler.reconcileResourceUpdate(subaccountId1, runtimeId11, runtimeStateType{betaEnabled: "true"})
		reconciler.reconcileCisAccount(subaccountId1, CisStateType{BetaEnabled: true, UsedForProduction: "NOT_USED", ModifiedDate: oldTime})
	...
	t.Run("should schedule two resource updates in proper order", func(t *testing.T) {
		...
		// then we get state from CIS (accounts)
		reconciler.reconcileCisAccount(subaccountId1, CisStateType{BetaEnabled: false, UsedForProduction: "false", ModifiedDate: veryOldTime})
