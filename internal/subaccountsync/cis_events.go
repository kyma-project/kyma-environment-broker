...
func (c *RateLimitedCisClient) FetchEventsWindow(fromActionTime int64) ([]Event, error) {
	...
	for {
		...
		if err != nil {
			c.log.Error(fmt.Sprintf("while fetching subaccount events for %d page: %v", currentPage, err))
			return nil, err
		}
		if currentPage == 0 {
			c.log.Info(fmt.Sprintf("event window from epoch: %d in %d pages", fromActionTime, cisResponse.TotalPages))
		}
	...
	c.log.Info(fmt.Sprintf("Fetched %d pages of events, with %d events", currentPage, len(events)))
	return events, nil
}
...
func (c *RateLimitedCisClient) getEventsForSubaccounts(fromActionTime int64, logs slog.Logger, subaccountsMap subaccountsSetType) ([]Event, error) {
	...
	if err != nil {
		logs.Error(fmt.Sprintf("error while fetching subaccount delete events: %s", err))
		return nil, err
	}

	// filter events to get only the ones in subaccounts map
	filteredEventsFromCis := filterEvents(rawEvents, subaccountsMap)
	logs.Info(fmt.Sprintf("Found %d raw events, filtered %d events of interest", len(rawEvents), len(filteredEventsFromCis)))
