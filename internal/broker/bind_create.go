...
func NewBind(cfg BindingConfig, db storage.BrokerStorage, log *slog.Logger, clientProvider broker.ClientProvider, kubeconfigProvider broker.KubeconfigProvider,
	publisher event.Publisher, useAdditionalOIDC, useMainOIDC, multipleContexts bool) *BindEndpoint {
	return &BindEndpoint{config: cfg,
		...
		serviceAccountBindingManager: broker.NewServiceAccountBindingsManager(clientProvider, kubeconfigProvider, useAdditionalOIDC, useMainOIDC, multipleContexts),
	}
}
