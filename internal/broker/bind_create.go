...
func NewBind(cfg BindingConfig, instanceStorage storage.Instances, log logrus.FieldLogger, kubeconfigSecretProvider broker.SecretProvider, tokenExpirationSeconds int) *BindEndpoint {
	return &BindEndpoint{config: cfg, instancesStorage: instanceStorage, log: log.WithField("service", "BindEndpoint"),
		bindings: broker.NewBindingsManager(kubeconfigSecretProvider, tokenExpirationSeconds)}
}
