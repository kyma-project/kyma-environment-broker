...
func TestCreateBindingEndpointEnvtest(t *testing.T) {
	...
	//// api handler
	bindEndpoint := NewBind(*bindingCfg, db, logs, skrK8sClientProvider, skrK8sClientProvider)
	getBindingEndpoint := NewGetBinding(logs, db)
	unbindEndpoint := NewUnbind(logs, db, brokerBindings.NewServiceAccountBindingsManager(skrK8sClientProvider, skrK8sClientProvider))
...
func createEnvTest(t *testing.T) (envtest.Environment, *rest.Config, client.Client) {
	...
	skrClient, err := client.New(config, client.Options{})
	require.NoError(t, err)
