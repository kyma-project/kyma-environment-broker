...
func TestProvision_Provision(t *testing.T) {
	...
	t.Run("Should accept an valid OIDC object type", func(t *testing.T) {
		...
		operation, err := memoryStorage.Operations().GetProvisioningOperationByID(response.OperationData)
		require.NoError(t, err)
		assert.Equal(t, "client-id", operation.ProvisioningParameters.Parameters.OIDC.ClientID)
		assert.Equal(t, "https://test.local", operation.ProvisioningParameters.Parameters.OIDC.IssuerURL)
		assert.Equal(t, []string{"RS256"}, operation.ProvisioningParameters.Parameters.OIDC.SigningAlgs)
		assert.Equal(t, []string{"claim=value"}, operation.ProvisioningParameters.Parameters.OIDC.RequiredClaims)
