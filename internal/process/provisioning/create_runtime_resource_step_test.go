...
func TestCreateRuntimeResourceStep_AWS_ZonesDiscovery(t *testing.T) {
	...
	cli := getClientForTests(t)
	step := NewCreateRuntimeResourceStep(memoryStorage, cli, inputConfig, defaultOIDSConfig, workers.NewProvider(broker.InfrastructureManager{}, fixture.NewProviderSpecWithZonesDiscovery(t, true), true), fixture.NewProviderSpecWithZonesDiscovery(t, true), config.GlobalAccountsConfig{}, nil)
...
func TestCreateRuntimeResourceStep_GvisorIsolation(t *testing.T) {
	for _, testCase := range []struct {
		...
		t.Run(testCase.name, func(t *testing.T) {
			...
			step := NewCreateRuntimeResourceStep(memoryStorage, cli, inputConfig, defaultOIDSConfig, workers.NewProvider(broker.InfrastructureManager{}, providerSpec, true), providerSpec, config.GlobalAccountsConfig{}, nil)
...
`,
	...
	kcrK8sClient := fake.NewClientBuilder().WithObjects(kcrConfigMap).Build()
	kcrProvider := provider.NewKCRVolumeProvider(kcrK8sClient, "consumption-reporter-config")
	step := NewCreateRuntimeResourceStep(memoryStorage, cli, inputConfig, defaultOIDSConfig, workers.NewProvider(broker.InfrastructureManager{}, fixture.NewProviderSpecWithZonesDiscovery(t, false), true), fixture.NewProviderSpecWithZonesDiscovery(t, false), config.GlobalAccountsConfig{}, kcrProvider)
