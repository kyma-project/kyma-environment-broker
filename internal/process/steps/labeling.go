...
func SetCommonLabels(labels map[string]string, operation internal.Operation) map[string]string {
	...
	labels[customresources.PlanNameLabel] = broker.AvailablePlans.GetPlanNameAsString(broker.PlanIDType(operation.ProvisioningParameters.PlanID))
...
func UpdatePlanLabels(labels map[string]string, planID string) map[string]string {
	...
	labels[customresources.PlanNameLabel] = broker.AvailablePlans.GetPlanNameAsString(broker.PlanIDType(planID))
	return labels
}
