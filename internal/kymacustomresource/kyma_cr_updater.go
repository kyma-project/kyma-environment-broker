...
func NewUpdater(k8sClient dynamic.Interface,
	...
	sleepDuration time.Duration,
	labelKey string, ctx context.Context, logger *slog.Logger) (*Updater, error) {
...
func (u *Updater) Run() error {
	for {
		u.logger.Debug("Checking if queue is empty")
		...
		contextWithTimeout, cancel := context.WithTimeout(u.ctx, requestTimeout)
		defer cancel()

		unstructuredList, err := u.k8sClient.Resource(u.kymaGVR).Namespace(namespace).List(contextWithTimeout, metav1.ListOptions{
		...
		for _, kymaCrUnstructured := range unstructuredList.Items {
			if err := u.updateBetaEnabledLabel(kymaCrUnstructured, item.BetaEnabled, contextWithTimeout); err != nil {
