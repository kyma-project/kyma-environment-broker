...
func TestBtpManagerReconciler(t *testing.T) {
	...
	t.Run("btp manager credentials tests", func(t *testing.T) {
		...
		t.Run("reconcile when all secrets are correct", func(t *testing.T) {
			...
			assert.Equal(t, updateNotDoneDueError+updateNotDoneDueOkState, expectedInstancesCount)
			assert.Zero(t, skippedCount)
			environment.assertAllSecretDataAreSet()
			environment.assureConsistency()
		})
		...
		t.Run("reconcile when some secrets are incorrect (randomly selected)", func(t *testing.T) {
			...
			assert.Equal(t, updateNotDoneDueError+updateNotDoneDueOkState, expectedInstancesCount-len(skrs))
			assert.Zero(t, skippedCount)
		...
		t.Run("reconcile when some secrets are incorrect (static selected)", func(t *testing.T) {
			...
			assert.Equal(t, updateNotDoneDueError+updateNotDoneDueOkState, expectedInstancesCount-len(testDataIndexes))
			assert.Zero(t, skippedCount)
		...
		t.Run("when one secret is labeled, changed and relabelled", func(t *testing.T) {
			...
			assert.Equal(t, expectedInstancesCount, instancesCount)
			assert.Equal(t, len(testDataIndexes)-1, updatedCount)
			assert.Equal(t, expectedInstancesCount-len(testDataIndexes), notUpdatedDueError+nothingToUpdate)
		...
		t.Run("reconcile when one secret is labeled with skip-reconciliation set not to true", func(t *testing.T) {
			...
			assert.Equal(t, expectedInstancesCount, instancesCount)
			assert.Equal(t, len(testDataIndexes), updatedCount)
			assert.Equal(t, expectedInstancesCount-len(testDataIndexes), updateNotDoneDueError+updateNotDoneDueOkState)
